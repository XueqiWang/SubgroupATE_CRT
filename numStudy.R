...
rho_s_name <- c("0.2" = "(A)~Vary~rho[y*'|'*s*','*z]~with~rho[s]==0.2", "0.5" = "(B)~Vary~rho[y*'|'*s*','*z]~with~rho[s]==0.5")
pplot_om_1 <- ggplot(data=pdata_om_1, aes(rho_y, pred.power, colour = factor(p_1), shape = factor(p_1))) +
  ...
  labs(title = "", #"Power of the omnibus test",
       y = "Power", x = expression(rho[y*'|'*s*','*z]),
...
rho_y_name <- c("0.05" = "(C)~Vary~rho[s]~with~rho[y*'|'*s*','*z]==0.05", "0.2" = "(D)~Vary~rho[s]~with~rho[y*'|'*s*','*z]==0.2")
